...
`glb_3dtiles_report.json` 會記錄 tile 數、目標大小、每顆 b3dm byte 數、geometry byte 估算、是否超過 target，以及外掛化貼圖數量與總 bytes。

`Terrain Remaked.glb` 實測命令使用 `--tile-target-bytes 2500000` 時，輸出 16 顆 b3dm，最大約 2.34 MiB；67 張內嵌貼圖會集中到 `tiles/textures/`，總量約 40.6 MiB。
