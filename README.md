...
GLB 轉檔會依 `--tile-target-bytes` 近似控制單顆 b3dm 大小，預設約 2.8MB。若遇到單一 indexed triangle primitive 過大，工具會在 primitive 內切 index range、重新 remap vertex、重建小 GLB，並重新計算每片 tile 的 POSITION bounds。這不是降面或壓縮畫質，而是把原本的大 geometry payload 分散成多顆 tile，讓 Cesium 串流載入更順。

貼圖處理：

- 來源 GLB 若使用內嵌 image bufferView，工具會把貼圖輸出一次到 `tiles/textures/`，各 tile GLB 以相對 URI 共用貼圖。
- 這可以避免每顆 b3dm 重複塞同一組貼圖，也是 `Terrain Remaked.glb` 從 74MB 單顆 b3dm 變成 2-3MB 多顆 b3dm 的關鍵。
- 來源 GLB 若原本使用外部 image URI，目前會保留該 URI；部署時需確認 texture 檔案相對於 tile URL 可被瀏覽器讀到。最穩的做法是先輸出成 embedded GLB，再交給本工具外掛化。
...
.\target\release\ifc_to_3dtiles.exe glb-to-3dtiles `
  ...
  --latitude 24.102594 `
  --height 0 `
  --tile-target-bytes 1500000 `
  --overwrite
```

`--longitude` / `--latitude` 是 WGS84 模型錨點；GLB 本身會保留原始局部座標，tileset root transform 負責把它放到地球上。

GLB 內容通常是 glTF Y-up；`glb_3dtiles_report.json` 會寫入 `source_up_axis` 與 `viewer_content_up_axis`。最新版 `out\html_template` 的 Three viewer 在 `contentUpAxis: "auto"` 時會讀這份 report；新版 `glb-to-3dtiles` 輸出預設標示為 viewer Z-up，不再額外旋轉。Three viewer 也會對 GLB material / primitive slice 主動載入 root children，避免房子外皮、路面、屋頂水塔或透明材質只載到一部分。
...
`glb_3dtiles_report.json` 會記錄 tile 數、目標大小、每顆 b3dm byte 數、geometry byte 估算、是否超過 target、座標軸提示，以及外掛化貼圖數量與總 bytes。

`Terrain Remaked.glb` 實測命令使用 `--tile-target-bytes 1500000` 時，輸出 28 顆 b3dm，最大約 1.41 MiB；67 張內嵌貼圖會集中到 `tiles/textures/`，總量約 40.6 MiB。Three viewer auto 模式可載入 28/28 個 material-slice tile，房子外皮、路面貼圖與屋頂水塔完整顯示。
